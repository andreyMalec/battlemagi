...
public class MultiDropdown : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler {
    protected internal class DropdownItem : MonoBehaviour, IPointerEnterHandler, ICancelHandler {
        ...
        public virtual void OnCancel(BaseEventData eventData) {
            MultiDropdown dropdown = GetComponentInParent<MultiDropdown>();
            if (dropdown)
                dropdown.Hide();
        }
    }
    ...
    public void RefreshShownValue() {
        ...
        if (m_CaptionText) {
            if (m_MultiSelect && data == null) {
                var selected = new List<string>();
                for (int i = 0; i < options.Count; i++) {
                    if ((m_Value & (1 << i)) != 0) selected.Add(options[i].text);
                }

                m_CaptionText.text = selected.Count == 0 ? "" : string.Join(", ", selected);
            } else {
        ...
        if (m_CaptionImage) {
            m_CaptionImage.sprite = data != null ? data.image : null;
            m_CaptionImage.color = data != null ? data.color : Color.white;
            m_CaptionImage.enabled = (m_CaptionImage.sprite != null && m_CaptionImage.color.a > 0);
        }
    ...
    private static int FirstActiveFlagIndex(int mask) {
        if (mask <= 0) return -1;
        var idx = 0;
        while ((mask & 1) == 0) {
            mask >>= 1;
    ...
    private static bool IsEverythingValue(int optionCount, int mask) {
        if (optionCount <= 0) return false;
        var all = (1 << optionCount) - 1;
        return (mask & all) == all;
    }

    private static bool IsFlagActive(int index, int mask) {
        if (index < 0) return false;
        return (mask & (1 << index)) != 0;
    }

    private static int SetFlag(int index, int mask, bool isOn) {
        if (index < 0) return mask;
        var bit = 1 << index;
        return isOn ? (mask | bit) : (mask & ~bit);
    }

    private int GetMask() {
        return m_MultiSelect ? m_Value : (m_Value < 0 ? 0 : (1 << m_Value));
    }

    private void SetMask(int mask, bool sendCallback = true) {
        if (!m_MultiSelect) {
            if (mask == 0) SetValue(-1, sendCallback);
            else SetValue(FirstActiveFlagIndex(mask), sendCallback);
            return;
        }

        if (Application.isPlaying && (mask == m_Value || options.Count == 0))
            return;
    ...
    public void ClearAll(bool sendCallback = true) {
        SetMask(0, sendCallback);
    }

    public void SelectAll(bool sendCallback = true) {
        var all = options.Count <= 0 ? 0 : (1 << options.Count) - 1;
        SetMask(all, sendCallback);
    }

    public bool IsSelected(int index) {
        return IsFlagActive(index, GetMask());
    }

    public void SetSelected(int index, bool isOn, bool sendCallback = true) {
        SetMask(SetFlag(index, GetMask(), isOn), sendCallback);
    }
    ...
    private static int EverythingValue(int optionCount) {
        ...
        if (m_MultiSelect) {
            switch (selectedIndex) {
                case 0: // Nothing
                    ...
                    break;
                case 1: // Everything
                    SetMask(EverythingValue(options.Count));
                    for (var i = 3; i < parent.childCount; i++) {
                        var toggleComponent = parent.GetChild(i).GetComponentInChildren<Toggle>();
                        if (toggleComponent)
                    ...
                    break;
                default:
                    var flagIndex = selectedIndex - 2;
                    var newMask = SetFlag(flagIndex, GetMask(), !IsFlagActive(flagIndex, GetMask()));
                    SetMask(newMask);

                    var isOn = IsFlagActive(flagIndex, GetMask());
                    toggle.SetIsOnWithoutNotify(isOn);

                    var nothing = parent.GetChild(1).GetComponentInChildren<Toggle>();
                    var everything = parent.GetChild(2).GetComponentInChildren<Toggle>();
                    if (nothing) nothing.SetIsOnWithoutNotify(GetMask() == 0);
                    if (everything) everything.SetIsOnWithoutNotify(IsEverythingValue(options.Count, GetMask()));
                    break;
            }
