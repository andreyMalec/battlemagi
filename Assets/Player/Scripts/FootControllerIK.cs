...
public class FootControllerIK : MonoBehaviour {
    ...
    public float FootHeightOffset {
        get => m_FootHeightOffset;
        set { m_FootHeightOffset = Mathf.Clamp(value, -HEIGHT_OFFSET, HEIGHT_OFFSET); }
    }

    [Header("Legs switching-on")] public bool leftEnabled = true;
    ...
    public float DistancePower {
        get => m_DistancePower;
        set { m_DistancePower = Mathf.Clamp(value, 0, MAX_DISTANCE_POWER); }
    }
    ...
    public float SmoothingAngle {
        get => m_SmoothingAngle;
        set { m_SmoothingAngle = Mathf.Clamp(value, 0, MAX_SMOOTHING_ANGLE); }
    }
    ...
    /// </summary>
    public float GlobalSmoothingPower {
        get => m_GlobalSmoothingPower;
        set { m_GlobalSmoothingPower = Mathf.Clamp(value, 0, SMOOTHING_POWER); }
    }
    ...
    /// </summary>
    [Space(5)]
    public RotationType rotationType = RotationType.RawTarget;
    ...
    private void Initiation() {
        ...
        Vector3 lowestPoint = transform.InverseTransformPoint((AnimatorBone(HumanBodyBones.LeftFoot).position +
                                                               AnimatorBone(HumanBodyBones.RightFoot).position) / 2);
    ...
    private void SetPositionRotationFromTarget(Limb limb, Transform target) {
        ...
        if (rotationType == RotationType.RawTarget) {
            ...
            limb.targetRotation = Quaternion.FromToRotation(transform.up, limb.UpBone.position - limb.targetPosition) *
        ...
        limb.canReachTarget = ((Vector3.Distance(limb.UpBone.position, limb.targetPosition) <=
                                (limb.UpperLength + limb.LowerLength + footHeight)));
    ...
    private void SetPositionRotationFromRayCast(Limb limb) {
        ...
        limb.AnimatorWeight = (Vector3.Dot(transform.up, limb.LowBonePosition - transform.position) -
                               limb.distanceFromMesh - m_MeshHeightOffset);

        if (type == CastType.Ray)
            low = Physics.Raycast(limb.LowBonePosition + transform.up * (m_MaxStepHeight - currentHeight),
                -transform.up, out hit, m_MaxStepHeight * 2, ~ignoreLayers);
        else if (type == CastType.Sphere)
            low = Physics.SphereCast(limb.LowBonePosition + transform.up * (m_MaxStepHeight - currentHeight),
                sphereRadius, -transform.up, out hit, m_MaxStepHeight * 2, ~ignoreLayers);
        else {
            low = Physics.Raycast(limb.LowBonePosition + transform.up * (m_MaxStepHeight - currentHeight),
                -transform.up, out hit, m_MaxStepHeight * 2, ~ignoreLayers);
            if (!low)
        ...
        if (increasedAccuracy && limb.ExtraBone) {
            ...
            if (type == CastType.Ray)
                extra = Physics.Raycast(limb.ExtraBone.position + transform.up * (m_MaxStepHeight - currentHeight),
                    -transform.up, out extraHit, m_MaxStepHeight * 2, ~ignoreLayers);
            else if (type == CastType.Sphere)
                extra = Physics.SphereCast(limb.ExtraBone.position + transform.up * (m_MaxStepHeight - currentHeight),
                    sphereRadius, -transform.up, out extraHit, m_MaxStepHeight * 2, ~ignoreLayers);
            else {
                extra = Physics.Raycast(limb.ExtraBone.position + transform.up * (m_MaxStepHeight - currentHeight),
                    -transform.up, out extraHit, m_MaxStepHeight * 2, ~ignoreLayers);
                if (!extra)
                    extra = Physics.SphereCast(
                        limb.ExtraBone.position + transform.up * (m_MaxStepHeight - currentHeight), sphereRadius,
                        -transform.up, out extraHit, m_MaxStepHeight * 2, ~ignoreLayers);
            }
        ...
        if (limb.canReachTarget) {
            ...
            if (pole) {
                ...
                limb.MiddleBone.rotation =
                    Quaternion.FromToRotation(LowerLeg, limb.targetPosition - limb.MiddleBone.position) *
                    limb.MiddleBone.rotation;
            } else {
                if (!fixKnee) {
                    planeThirdPoint = (limb.MiddleBone.position + limb.LowBone.position) / 2 +
                ...
                limb.UpBone.rotation = Quaternion.FromToRotation(limb.LowBonePosition - limb.UpBone.position,
    ...
    private void FootsPlacement(Limb limb, Transform target, Transform pole) {
        ...
        if (pole)
            ...
            if (!fixKnee)
                ...
                if (limb == m_RightLeg && fromGlobalToLocaltarget.x <= fromGlobalToLacalUp.x &&
                    fromGlobalToLocaltarget.z <= fromGlobalToLacalUp.z)
                    ang = 180 - ang;

                if (limb == m_LeftLeg && fromGlobalToLocaltarget.x >= fromGlobalToLacalUp.x &&
                    fromGlobalToLocaltarget.z <= fromGlobalToLacalUp.z)
                    ang = 180 - ang;
    ...
    private class Limb {
        ...
        public float MiddleBoneAngle(float targetDistance) {
            return Mathf.Acos(Mathf.Clamp(
                       (CosineRuleNumeratorPart - (targetDistance * targetDistance)) / CosineRuleDenominator, -1, 1)) *
                   Mathf.Rad2Deg;
        }
